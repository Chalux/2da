...
func _get_dotnet_dialogue_manager() -> Node:
	#return load(get_script().resource_path.get_base_dir() + "/Dialogic.cs").new()
	var script :CSharpScript = load("res://addons/dialogic/Dialogic.cs");
	return script.new()
